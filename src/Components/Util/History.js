  ...
  return (
    <div className={s1}>
      <div className="text-right">
        ...
        >
          {choices.map((e) => (
            <option key={e} value={e} selected={status === e}>
              {e}
            </option>
          ))}
        </select>
      ...
      <table className="rounded-md">
        <thead>
          {props.user === "bank" ? (
            <tr>
              <th className="border p-4 px-4">
                {props.handle === "donations" ? "Donor" : "Patient"} Name
              </th>
              <th className="border p-2 px-4">Age</th>
              <th className="border p-4 px-4">Food Group</th>
              <th className="border p-4 px-4">Gender</th>
              <th className="border p-4 px-4">Units(kg)</th>
              <th className="border p-4 px-4">
                {props.handle === "donations" ? "Disease" : "Reason"}
              </th>
            ...
            <tr>
              <th className="border p-4 px-7">Units(kg)</th>
          ...
          {data.map((e, i) =>
            props.user === "bank" ? (
              <tr
                key={i}
                className={
                  status === "All" ? "" : status !== e.status ? "hidden" : ""
                }
              >
                <td
                  className="border underline decoration-dotted underline-offset-4 cursor-pointer p-3"
                  onClick={() => setPopup(i)}
                >
                  {props.handle === "donations" ? e.userId.name : e.name}
                </td>
                <td className="border p-3">
                  {props.handle === "donations" ? e.userId.age : e.age}
                </td>
                <td className="border p-3">
                ...
                <td className="border p-3">
                  {props.handle === "donations"
                    ? e.userId.gender[0].toUpperCase() +
                      e.userId.gender.substr(1)
              ...
              >
                ...
                <td
                  className="border p-3 cursor-pointer underline decoration-dotted underline-offset-4"
                  onClick={() => setPopup(i)}
                >
                  {e.bankId.name}
                </td>
